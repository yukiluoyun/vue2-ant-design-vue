...
var drivingRoute, drivingRoute1
var _CarTrack
var startIcon = require('@/assets/images/rank_5.png') // 起点图标
var endIcon = require('@/assets/images/rank_6.png') // 终点图标
...
export const mapMixins = {
  methods: {
    ...
    renderLine(overLay, dataSource, lineColor) {
      // if (overLay === 'supportOverLay') {
      //   // 支撑段 --- 现在因为数据异常，看不到效果，暂时屏蔽掉这种正确的写法
      //   this[overLay] = new Array(dataSource.length).fill(null)
      //   dataSource.forEach((item, index) => {
      //     const coordinatesGroup = item.coordinates.split(' ')
      //     // 第一组经纬度
      //     const coordinatesItem1 = new window.T.LngLat(
      //       coordinatesGroup[0][0],
      //       coordinatesGroup[0][1]
      //     )
      //     // 第一组经纬度
      //     const coordinatesItem2 = new window.T.LngLat(
      //       coordinatesGroup[1][0],
      //       coordinatesGroup[1][1]
      //     )
      //     const points = [coordinatesItem1, coordinatesItem2]
      //     // 创建线对象

      //     this[overLay][index] = new window.T.Polyline(points)
      //     // console.log('this[overLay][index]==', this[overLay][index])
      //     // 向地图上添加线
      //     this.map.addOverLay(this[overLay][index])
      //   })
      // } else {
      //   const points = []
      ...
      // 线条轨迹
      this.$nextTick(() => {
        this.track(points, dataSource)
      })

      // 鼠标经过线条事件
      lineMouseoverHandler(this[overLay], dataSource)

      // 鼠标移动时，坐标点打开一个窗口
      function lineMouseoverHandler(overLay, dataSource) {
        overLay.addEventListener('mouseover', function (e) {
          console.log('lineMouseoverHandler', dataSource)
          // lineOpenInfo(that, content, e)
        })
      }

      // 开启信息窗口
      // function lineOpenInfo(that, content, e) {
      //   var point = e.lnglat
      //   marker = new window.T.Marker(point) // 创建标注
      //   var markerInfoWin = new window.T.InfoWindow(content, {
      //     offset: new window.T.Point(0, -30)
    ...
    // 轨迹
    track(points, dataSource) {
      var startLngLat = points[0]
      // var endLngLat = points[dataSource.length - 1]
      var endLngLat = points[1]

      var config = {
        policy: 0, // 驾车策略  0 最短距离 3 步行
        onSearchComplete: this.searchResult // 检索完成后的回调函数
      }
      drivingRoute = new window.T.DrivingRoute(this.map, config)
      // drivingRoute1 = new window.T.DrivingRoute(this.map, config)
      this.searchDrivingRoute(startLngLat, endLngLat)

      // console.log('drivingRoute==', drivingRoute)
    },
    // 轨迹 -2 --检索完成后的回调函数
    searchResult(result) {
      console.log('检索完成后的回调函数--searchResult-内部==', result)
      // 添加起始点
      this.createStartMarker(result)
      const obj = result
      // 获取方案个数
      var routes = result.getNumPlans()
      console.log('所有路线==', routes)
    ...
    // 轨迹-1
    searchDrivingRoute(startLngLat, endLngLat) {
      console.log('指定路线 从起始点到结束点 ==', drivingRoute)
      // this.map.clearOverLays()
      // var startLngLat = new window.T.LngLat(116.35406, 39.90565)
      // var endLngLat = new window.T.LngLat(116.42813, 39.90355)
      // 驾车路线搜索
      drivingRoute.search(startLngLat, endLngLat)

      // var startLngLat1 = new window.T.LngLat(116.380967, 39.953285)
      // var endLngLat1 = new window.T.LngLat(116.424374, 39.914668)
      // 驾车路线搜索
      // drivingRoute1.search(startLngLat1, endLngLat1)
    },
    createRoute(lnglats, lineColor) {
      var _CarTrack = new window.T.CarTrack(this.map, {
        interval: 20,
        speed: 10,
        dynamicLine: false,
        Datas: lnglats,
    ...
    createStartMarker(result) {
      var startMarker = new window.T.Marker(result.getStart(), {
        icon: new window.T.Icon({
          iconUrl: startIcon,
      ...
      var endMarker = new window.T.Marker(result.getEnd(), {
        icon: new window.T.Icon({
          iconUrl: endIcon,
          iconSize: new window.T.Point(44, 34),
          iconAnchor: new window.T.Point(12, 31)
